...
class ListEventAdminPage extends StatefulWidget {
  @override
  _ListEventAdminPageState createState() => _ListEventAdminPageState();
}

class _ListEventAdminPageState extends State<ListEventAdminPage> {
                ...
                builder: (_, eventState) {
                  if (eventState is EventLoaded) {
                    List<EventInfo> listEvent = eventState.listEvent
                        .where(
                          (element) =>
                              element.status == StatusEvent.panding &&
                              ...
                              return Container(
                                ...
                                child: GestureDetector(
                                    onTap: () async {
                                      navigatorPage(
                                        context,
                                        navigator: NavigatorPage.push,
                                        to: DetailEventAdminPage(
                                            listEvent[index]),
                                      );
                                    },
                                    child:
                    ...
                    builder: (_, eventState) {
                      if (eventState is EventLoaded) {
                        List<EventInfo> listEvent = eventState.listEvent
                            .where(
                              (element) =>
                                  element.status == StatusEvent.publish &&
                                  element.timeReglimit.isAfter(
                                    DateTime.now(),
                                  ),
                            )
                ...
                builder: (_, eventState) {
                  if (eventState is EventLoaded) {
                    List<EventInfo> listEvent = eventState.listEvent
                        .where(
                          (element) =>
                              element.status == StatusEvent.cenceled ||
